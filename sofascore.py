...
# Only the ids above that have a matching LEAGUE_* toggle in .env get
# checked against it here — the rest (Champions League, World Cup,
# AFCON, FA Cup, Liga MX) have no toggle at all and stay always-on,
# same as before. Keys must match config.WATCHED_LEAGUES exactly.
_TOURNAMENT_ID_TO_LEAGUE_KEY = {
    17:  "Premier League",
    8:   "La Liga",
    35:  "Bundesliga",
    23:  "Serie A",
...
# Previously a "secondary" tier: fetched, but capped at
# MAX_SECONDARY_MATCHES per poll and restricted to a full-time-only
# result post. Turned off entirely below (folded into the exclude
# list) — "Club Friendly Games" (853) in particular covers a huge grab
# bag of largely unfamiliar lower-league/reserve/exhibition matches
# worldwide, not just recognizable club friendlies, so it was adding
# noise nobody following the page actually wants. Left here, unused,
# in case you want to bring a specific competition back — just move
# its id into PRIORITY_TOURNAMENT_IDS above instead of re-enabling
# this tier.
SECONDARY_TOURNAMENT_IDS: set[int] = set()

_EXCLUDED_TOURNAMENT_IDS = {
    853,  # Club Friendly Games (World) — mostly unfamiliar clubs
...
def _tournament_tier(tournament: dict) -> str | None:
    ...
    if unique_tournament_id in _EXCLUDED_TOURNAMENT_IDS:
        return None
    if unique_tournament_id in PRIORITY_TOURNAMENT_IDS:
        league_key = _TOURNAMENT_ID_TO_LEAGUE_KEY.get(unique_tournament_id)
        if league_key is not None and not config.WATCHED_LEAGUES.get(league_key, config.WATCHED_LEAGUES_DEFAULT):
            return None
        return "priority"
...
def _normalize_sofascore(event: dict) -> dict | None:
    try:
        ...
        def _watched_and_league_on(team_id: str) -> bool:
            ...
            if league is None:
                return True
            return config.WATCHED_LEAGUES.get(league, config.WATCHED_LEAGUES_DEFAULT)
...
def get_lineup(event_id: str) -> list[dict]:
    ...
    try:
        ...
        for side in ("home", "away"):
            ...
            team_name = team_data.get("name") or (team_data.get("team", {}) or {}).get("name", "?")
